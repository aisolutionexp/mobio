...
import { changeRetailerPlan } from "@/lib/actions/subscriptions-retailer";
import type { PlanData } from "@/lib/actions/subscriptions";
import { Plate } from "@/components/editorial/plate";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";

interface RetailerPlanCardProps {
  plan: PlanData;
  isCurrent: boolean;
  canChange: boolean;
}
...
export function RetailerPlanCard({
  plan,
  isCurrent,
  canChange,
}: RetailerPlanCardProps) {
  const [isPending, startTransition] = useTransition();

  function handleChange() {
    if (!confirm(`Deseja trocar para o plano ${plan.name}?`)) return;
    startTransition(async () => {
      const result = await changeRetailerPlan(plan.id);
      if (result.success) {
