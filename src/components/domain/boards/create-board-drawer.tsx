...
export function CreateBoardDrawer({ children }: { children: React.ReactNode }) {
      ...
      <Dialog.Portal>
        ...
        <Dialog.Popup className="bg-card border-border fixed inset-y-0 right-0 z-60 flex w-full max-w-md flex-col border-l shadow-lg">
          <div className="border-border flex items-center justify-between border-b px-6 py-4">
            <Eyebrow as="span">Novo board</Eyebrow>
            <Dialog.Close
          ...
          <form action={handleSubmit} className="flex flex-1 flex-col">
            <div className="flex-1 space-y-4 overflow-y-auto p-6">
              <TextInput
                name="name"
                label="Nome do board"
                placeholder="Ex: Seleção inverno 2027"
                required
                minLength={1}
                maxLength={100}
              ...
              <Field>
                ...
                <FieldControl>
                  <Textarea
                    id="description"
                    name="description"
                    placeholder="Descreva o propósito deste board (opcional)"
                    maxLength={500}
                    showCount
                    autoResize
                  />
